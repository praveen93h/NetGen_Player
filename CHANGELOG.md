...
- **Online Subtitle Download** - Documented the subtitle discovery and download workflow for finding matching captions
- **Subtitle Customization** - Documented styling controls for font size, background, color, outline, and shadow
- **Dual Subtitles** - Documented primary and secondary subtitle workflows for bilingual viewing
- **Subtitle Engine Positioning** - Updated docs to highlight timing, styling, and positioning preservation in subtitle rendering

### Changed
- Version bumped to 1.6.0 (versionCode 8)
- README version badge updated to 1.6.0
- Roadmap updated to mark v1.6 as Done
- README and changelog punctuation normalized to clean ASCII
...
- **GestureController** - Fixed brightness gesture crash; replaced Application-to-Activity context cast with explicit `attachActivity()`/`detachActivity()` lifecycle pattern
...
### Known Issues
- Server bookmark credentials are stored in plaintext in Room DB (planned for encryption in a future release)
